...
class GooiRequestController extends AbstractController
{
    ...
    public function new()
    {
        ...
        $gooiRequest->setIBAN('Vul hier je rekeningnummer in');//Deze tekst komt dus in het invulblok te staan, moet de gebruiker dan weghalen om zijn data in te vullen
        ...
        if($form->isSubmitted() && $form->isValid())
        {
            ...
            $email = new GooiRequestMail($gooiRequest);
            $email->send();

            if($email->emailSent)
            {
                //TODO: toon de gebruiker dat zijn verzoek verzonden is
